...
| No policy for a category | Handled | Balance API returns `has_policy: false`; requests fail clearly; seed includes Maternity with no policy. |
| Mid-period joiner | Handled | Eligible days prorate the first period; `test_a_mid_period_joiner_is_prorated_by_eligible_calendar_days`. |
...
| Partial time spans several days | Decided | Rejected rather than guessing allocation. |
| Overlapping live request | Handled | Pending and approved ranges block overlap. |
...
| Two admins approve concurrently | Handled | Snapshot is selected `FOR UPDATE`; debit source key is unique. |
...
| Holiday changes after submission | Decided | Frozen request-day costs remain; reconciliation is a future admin action. |
